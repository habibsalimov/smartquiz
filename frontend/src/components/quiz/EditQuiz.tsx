...
const EditQuiz: React.FC = () => {
  ...
  const fetchQuizData = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.error || 'Quiz yüklenemedi');
      }
    } catch (error) {
  ...
  }, [user, navigate, quizId, fetchQuizData]);
  ...
  const handleAIQuizGenerated = (aiQuiz: any) => {
    setQuizData(prev => ({
      ...prev,
      questions: [...prev.questions, ...aiQuiz.questions]
    }));
  ...
  const saveQuiz = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate('/manage-quizzes', { state: { message: 'Quiz başarıyla güncellendi!' } });
      } else {
        const errorData = await response.json();
        setError(errorData.error || 'Quiz güncellenemedi');
      }
    } catch (error) {
  ...
  const renderStepContent = () => {
    switch (currentStep) {
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-purple-600 text-sm mb-3">
                    Mevcut quiz'e AI ile yeni sorular ekleyebilirsiniz
                  </p>
                </div>
                <div>
                  <button
                    ...
                    className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-200 transform hover:scale-105"
                  >
                    🚀 AI Soru Ekle
                  </button>
                </div>
      ...
      case 3:
        return (
          <div className="space-y-6">
            <h2 className="text-2xl font-bold mb-4">İncele ve Kaydet</h2>
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow p-8">
          ...
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-3xl font-bold">Quiz Düzenle</h1>
            <button
